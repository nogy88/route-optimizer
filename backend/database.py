...
#  Tables:
#    datasets   – uploaded file sets (stores blob excluded; matrix stored)
#    stores     – individual store records (editable)
#    vehicles   – individual vehicle records (editable)
#    jobs       – optimization run history
#    job_results – full result JSON + Excel blob
# ============================================================
...
from sqlalchemy import (
    ...
    DateTime, Text, LargeBinary, ForeignKey, event
)
...
class Dataset(Base):
    ...
    matrix_bytes = Column(LargeBinary, nullable=True)   # raw Excel bytes
...
class Store(Base):
    ...
    mn_name     = Column(String(200))
    address     = Column(String(400))
    detail_addr = Column(String(400))
    lat         = Column(Float)
    lon         = Column(Float)
    open_s      = Column(Integer, default=0)
    close_s     = Column(Integer, default=86399)
    dry_cbm     = Column(Float, default=0.0)
    dry_kg      = Column(Float, default=0.0)
    cold_cbm    = Column(Float, default=0.0)
    cold_kg     = Column(Float, default=0.0)
    has_dry     = Column(Boolean, default=False)
    has_cold    = Column(Boolean, default=False)
    ...
    def to_solver_dict(self):
        return {
            ...
            "eng_name"   : self.eng_name or "",
            "mn_name"    : self.mn_name  or "",
            "address"    : self.address  or "",
            "detail_addr": self.detail_addr or "",
            "lat"        : self.lat,
            "lon"        : self.lon,
            "open_s"     : self.open_s,
            "close_s"    : self.close_s,
            "dry_cbm"    : self.dry_cbm,
            "dry_kg"     : self.dry_kg,
            "cold_cbm"   : self.cold_cbm,
            "cold_kg"    : self.cold_kg,
            "has_dry"    : self.has_dry,
            "has_cold"   : self.has_cold,
        }
...
class Vehicle(Base):
    ...
    fleet        = Column(String(10))   # DRY or COLD
    cap_kg       = Column(Float)
    cap_m3       = Column(Float)
    fuel_cost_km = Column(Float)
    ...
    def to_solver_dict(self):
        return {
            ...
            "cap_kg"       : self.cap_kg,
            "cap_m3"       : self.cap_m3,
            "fuel_cost_km" : self.fuel_cost_km,
...
class Job(Base):
    ...
    def to_dict(self):
        return {
            ...
            "rural_solver_time": self.rural_solver_time,
            "status"      : self.status,
...
class JobResult(Base):
    ...
    unserved_json  = Column(Text)
    map_data_json  = Column(Text)
    excel_bytes    = Column(LargeBinary)
